document.addEventListener('DOMContentLoaded', async () => {
    const gridContainer = document.getElementById('trip-list-body');
       ...
       const response = await fetch('/api/trips/favorite', {
        ...
        if (plans.length > 0) {
            plans.forEach(planner => {
                ...
                const rowHTML = `
                    <tr onclick="location.href='/detail/${planner.id}'" style="cursor: pointer;">
        ...
        else 
        {
            gridContainer.innerHTML = `
                <tr>
                    <td colspan="4" class="empty-row">'${currentUser.country}' 관련 여행 플랜이 없습니다.</td>
                </tr>
            `;
        }
    } 
